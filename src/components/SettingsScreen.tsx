...
type SettingsKey = 'push' | 'email' | 'sms' | 'promos';
...
export const SettingsScreen: React.FC = () => {
  ...
  const options: { id: SettingsKey; icon: React.ElementType; title: string; desc: string }[] = [
          ...
          {options.map(opt => (
            <div key={opt.id} className="flex items-start justify-between gap-4">
              ...
              <button
                ...
                className={`w-14 h-8 rounded-full transition-colors relative shrink-0 ${settings[opt.id] ? 'bg-primary' : 'bg-slate-200'}`}
              >
                <div className={`w-6 h-6 bg-white rounded-full absolute top-1 transition-transform shadow-sm ${settings[opt.id] ? 'left-7' : 'left-1'}`} />
              </button>
            </div>
          ))}
        </div>
