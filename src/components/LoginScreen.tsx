...
import { Mail, Lock, Apple, Smartphone, User, Phone, AlertCircle } from 'lucide-react';
import { api, authStorage } from '../services/api';

export const LoginScreen: React.FC<{ onLogin: (rol: string) => void, onNavigateToComingSoon: () => void }> = ({ onLogin, onNavigateToComingSoon }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let respuesta;
      if (mode === 'login') {
        respuesta = await api.auth.login(email, password);
      } else {
        respuesta = await api.auth.registro({ nombre, email, password, telefono, rol: role });
      }
      authStorage.setToken(respuesta.access_token);
      authStorage.setUser(respuesta);
      onLogin(respuesta.rol);
    } catch (err: any) {
      ...
      <div className="w-full max-w-[120rem] grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        <section className="flex flex-col items-center lg:items-start space-y-10">
          ...
          <div className="w-full max-w-md bg-white rounded-[3rem] p-8 shadow-editorial">
            ...
            {error && (
              <div className="flex items-center gap-3 p-4 bg-red-50 border border-red-100 rounded-2xl text-red-700 text-sm font-medium animate-in fade-in">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              ...
              <button type="submit" className="btn-primary w-full text-lg disabled:opacity-60" disabled={loading}>
