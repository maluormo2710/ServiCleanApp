...
import { Star, MapPin, ArrowLeft, Verified, ShieldCheck, Calendar, MessageCircle, X, CreditCard, CheckCircle, ChevronDown, Loader2 } from 'lucide-react';
import { Address, PaymentMethod } from '../types';
import { api, ColaboradorOut, ResenaOut } from '../services/api';

interface WorkerProfileProps {
  workerId: number;
  onBack: () => void;
  onBookingSuccess: () => void;  // navigate to bookings after success
  addresses: Address[];
...
export const WorkerProfile: React.FC<WorkerProfileProps> = ({ workerId, onBack, onBookingSuccess, addresses, paymentMethods, onNavigateToChat }) => {
  const [worker, setWorker] = useState<ColaboradorOut | null>(null);
  const [loadingWorker, setLoadingWorker] = useState(true);
  const [submittingBooking, setSubmittingBooking] = useState(false);

  useEffect(() => {
    setLoadingWorker(true);
    api.colaboradores.obtener(workerId)
      .then(setWorker)
      .catch(console.error)
  ...
  const defaultServiceType = SERVICE_TYPES.find(
    s => worker?.especialidad.includes(s.name) || s.name.includes(worker?.especialidad || '')
  ) || SERVICE_TYPES[0];
  ...
  if (!worker) return (
    <div className="min-h-screen flex items-center justify-center">
      <p className="text-on-surface-variant">Colaborador no encontrado.</p>
    </div>
  );
  ...
  const calculatePrice = () => {
    ...
    if (hours <= 0) return worker.tarifa_hora; // Fallback a 1 hora
    return hours * worker.tarifa_hora;
  };
  ...
  const handleFinishBooking = async () => {
    const hours = calculateTimeDiffInHours() || 1;
    setSubmittingBooking(true);
    try {
      await api.reservas.crear({
        colaborador_id: worker.id,
        fecha: bookingData.date || new Date().toISOString().split('T')[0],
        hora: bookingData.time || '09:00',
        tipo_servicio: bookingData.type,
        direccion: bookingData.address,
        duracion_horas: hours,
        precio: calculatedPrice,
        tareas: bookingData.tasks.length > 0 ? bookingData.tasks : undefined,
        comentarios_cliente: bookingData.comments || undefined,
      });
      setBookingStep(0);
      onBookingSuccess();
    } catch (e: any) {
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 mb-1">
                ...
                <div className="flex items-center gap-1 text-amber-600 font-bold">
                  ...
                  <span className="text-lg">{worker.calificacion_promedio.toFixed(1)}</span>
                  <span className="text-on-surface-variant font-normal text-sm">({worker.servicios_completados} servicios)</span>
                </div>
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          ...
          <div className="bg-primary text-white p-8 rounded-[2rem] flex flex-col justify-between">
            <div>
              ...
              <p className="text-5xl font-black mt-2">{worker.servicios_completados}</p>
            </div>
            <div className="mt-4">
        ...
        {worker.resenas && worker.resenas.length > 0 && (
          <section className="mb-20">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-2xl font-bold flex items-center gap-3">Reseñas de Clientes</h3>
              <button onClick={() => setShowReviewsModal(true)} className="text-primary font-bold text-sm hover:underline">
                Ver todas
              </button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {worker.resenas.slice(0, 4).map((resena) => (
                  ...
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold">
                        {resena.autor_nombre.charAt(0)}
                      </div>
                      <div>
                        <h4 className="font-bold text-on-surface">{resena.autor_nombre}</h4>
            ...
            {bookingStep === 3 && (
              <div className="text-center py-8 animate-in zoom-in-95 duration-300">
                ...
                <button
                  ...
                  className="w-full py-4 rounded-full font-bold text-lg bg-primary text-white shadow-editorial active:scale-95 transition-transform flex items-center justify-center gap-2 disabled:opacity-60"
                >
                  {submittingBooking && <Loader2 size={20} className="animate-spin" />}
                  {submittingBooking ? 'Guardando...' : 'Ver mis reservas'}
                </button>
              </div>
            )}
            </motion.div>
          ...
          >
            ...
            >
              ...
              <div className="overflow-y-auto pr-2 space-y-4 no-scrollbar flex-1">
                {worker.resenas?.map((resena: ResenaOut) => (
